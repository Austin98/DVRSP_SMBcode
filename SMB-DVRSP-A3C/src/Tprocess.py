        ...
        for _ in range(opt.num_local_steps):
            ...
            if done:
                ...
                if save:
                    ...
                    sh.write(game_count, 3, info['coins'])
                    sh.write(game_count, 4, int(info['flag_get']))
                    sh.write(game_count, 5, int(info['life']))
                    sh.write(game_count, 6, info['score'])
                    sh.write(game_count, 7, int(info['stage']))
                    sh.write(game_count, 8, info['status'])
                    sh.write(game_count, 9, time_max - int(info['time']))
                    sh.write(game_count, 10, int(info['world']))
                    sh.write(game_count, 11, int(info['x_pos']))
                    sh.write(game_count, 12, int(info['y_pos']))
                    ...
                    if game_count == 1:
                        time_last5 = start_time
                        step_use5 = 0
                        step_last5 = 0
                        time_left5 = 10000
                    if game_count % 5 == 0:
                        time_use5 = timeit.default_timer() - time_last5
                        step_use5 = total_step - step_last5
                        time_last5 = timeit.default_timer()
                        step_last5 = total_step
                    if game_count > 5:
                        time_left5 = time_use5 / step_use5 * step_left / 60

                    print("\rW{}S{}-{}使用{}运行生成了【{}】条数据，用时:{:.2f}秒，进度: {:.2f}%，预计{:.2f}分钟({:.2f}小时)后完成".format(
                        opt.world, opt.stage, filename, gpu_or_cpu, game_count, time_use, per, time_left5,
                        time_left5 / 60), end='')
            ...
            if game_count == 0:
                ...
                max_xpos = max(DED)
            delta_reward += int(info['x_pos']) / max_xpos

            """静止惩罚SF"""
            if opt.use_SF and game_count > ss:
                D_DED = np.var(DED[-ss:])
                D_TED = np.var(TED[-ss:])
                if D_DED + D_TED == 0 and flag_get_rate < 0.2:
                    delta_reward = -1
                elif D_DED + D_TED == 0:
                    delta_reward = 0
                else:
            ...
            if game_count > 100 and flag_get_rate > 0.5:
                delta_reward = 0
            ...
            #     if vr_reward > 2:  # 2
            #         vr_reward = 2
            #     elif vr_reward < 0:  # 0.5
            #         vr_reward = 0
            #     vr_reward = (vr_reward - 1) / 5
            #
            # reward = (reward + delta_reward) + vr_reward
