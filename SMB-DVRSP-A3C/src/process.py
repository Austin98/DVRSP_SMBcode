        ...
        for _ in range(opt.num_local_steps):
            ...
            if done:
                ...
                if save:
                    ...
                    print("\r{}使用{}运行生成了【{}】条数据，用时:{:.2f}秒，进度: {:.2f}%，预计{:.2f}分钟后完成".format(
                        opt.data_label, gpu_or_cpu, game_count, time_use, per, time_left), end='')
            ...
            if opt.use_VR and game_count > vs:
                ...
                vr_reward = int(info['x_pos']) / (curr_step + 1) / Base_Velocity
                if vr_reward > 1.4:
                    delta_reward += 0.1
                    if vr_reward > 2:
                        delta_reward += 0.1
